...
def label_and_extract_features():
    global refPt, cnt_refPt, image_backup, creating_spot, image, samples_a, samples_b, samples_H, labels
    ...
    # a mask has to be passed in one of the parameters, in order to do this, we passed only one of the channels of the mask created in the previous section     #
    #-----------------------------------------------------------------------------------------------------------------------------------------------------------#    
        HSV_roi = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)                          # converting image from BGR space to HSV color space 

        hist_H = cv2.calcHist([HSV_roi],[0],mask[:,:,2],[32],[0,180])           # computing histograms for channel H
        hist_H = hist_H.flatten()                                               # histograms is converted to its appropiate format
        samples_H.append(hist_H)                                                # and then stored in its respective sample array
...
if __name__ == "__main__":
    ...
    outimage = "training_files/" + outfile + ".jpg"
    outfile = "training_files/" + outfile + ".npz"
    np.savez(outfile, labels=labels, samples_a=samples_a, samples_b=samples_b, samples_H=samples_H, samples_f=samples_f)
