...
def update_samples(img):
    ...
    #-----------------------------------------------------------------------------------------------------------------------------------------------------------#    
        ...
        hist_H = cv2.calcHist([HSV_roi],[0],masks[i][:,:,2],[32],[0,255])       # computing histogram for channels H
    ...
    return np.float32(samples_a), np.float32(samples_b), np.float32(samples_H), np.float32(samples_f)

def draw_rectangles(img, group, msg):
    global refPt, masks, min_x, min_y, max_x, max_y, x_size, y_size, cnt
    ...
    outfile = 'testing_outputs/' + msg + '_result' + str(cnt) + '.jpg'
...
def main():
    ...
    cnt = 0
    for filename in glob.glob('testing_pictures/*.jpg'):
        image = cv2.imread(filename)
        samples_a, samples_b, samples_H, samples_f = update_samples(image)
        ...
        draw_rectangles(image, result_H, 'channel H')
        draw_rectangles(image, result_f, 'fast')
        cnt = cnt + 1
        cv2.waitKey(4000)
